...
@app_commands.describe(
    ...
    image_url="Optional image URL to show in the embed."
)
...
async def welcomesetup(
    ...
    embed_color = get_embed_color(color_name)
    ...
    if use_embed:
        ...
        await interaction.response.send_message(
            ...
            "**Embed:** Yes\n"
            f"**Color:** `{color_name}`\n"
            f"**Image:** `{image_url.strip() or 'None'}`",
            ephemeral=True
        )
...
async def role(
    ...
    # Cannot give role above/equal to bot's highest role
